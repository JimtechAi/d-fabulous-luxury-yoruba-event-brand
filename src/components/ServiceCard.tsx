...
export const ServiceCard: React.FC<ServiceCardProps> = ({ service, className = '' }) => {
  const serviceVideoMap: Record<string, string> = {
    'alaga-iduro': '/assets/services/alaga-iduro/alaga-iduro.webp.mp4',
    'alaga-ijoko': '/assets/services/alaga-ijoko/alaga-ijoko.webp.mp4',
  };

  const serviceVideoSrc = serviceVideoMap[service.id];
  ...
  return (
    <article className={`group relative bg-ivory-warm border border-burgundy-deep/15 hover:border-gold-luxury/70 p-6 sm:p-8 transition-all duration-500 hover:shadow-xl flex flex-col justify-between ${className}`}>
      ...
      <div>
        ...
        <div className="relative w-full h-40 mb-6 bg-burgundy-dark/95 border border-gold-luxury/20 overflow-hidden flex flex-col items-center justify-center p-4 text-center group-hover:border-gold-luxury/40 transition-colors">
          {serviceVideoSrc ? (
            <video
              src={serviceVideoSrc}
              muted
